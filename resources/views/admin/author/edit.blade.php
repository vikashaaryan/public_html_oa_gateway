              ...
              <div class="row">
                ...
                <div class="col-lg-6">
                  <div class="mb-3">
                      ...
                      <input type="text" id="email" name="email" placeholder="{{__('message.enter_email')}}" class="form-control" autofocus value="{{$data->email}}">
                  </div>
                </div>
                <div class="col-lg-6">
                  <div class="mb-3">
                      ...
                      <select name="location" id="location" class="form-control" autofocus>
                        ...
                        @if($locations->isNotEmpty())
                          @foreach ($locations as $location_1)
                            <option value="{{$location_1->id}}" @if($data->location_id== $location_1->id) selected @endif>{{$location_1->name}}</option>
                          @endforeach
                        @endif
                      </select>
                  </div>
                      ...
                      <select name="job" id="job" class="form-control" autofocus>
                        ...
                        @if($jobs->isNotEmpty())
                          @foreach ($jobs as $job_1)
                            <option value="{{$job_1->id}}"  @if($data->job_id== $job_1->id) selected @endif>{{$job_1->name}}</option>
                          @endforeach
                        @endif
                      </select>
                  </div>
                      ...
                      <select name="university" id="university" class="form-control" autofocus>
                        ...
                        @if($universities->isNotEmpty())
                          @foreach ($universities as $university_1)
                            <option value="{{$university_1->id}}" @if($data->university_id== $university_1->id) selected @endif>{{$university_1->name}}</option>
                          @endforeach
                        @endif
                      </select>
                  </div>
                ...
                <div class="col-lg-6">
                  <div class="mb-3">
                      ...
                      <textarea  id="description2" name="description" style="display:none">{!! $data->about !!}</textarea>
                  </div>
                </div>
                <div class="col-lg-6">
                  <div class="mb-3">
...
@php
$rules = 'name: {required: true,minlength: 3,maxlength: 150},image:{required:false},email:{required:true,email:true},location:{required:true},job:{required:true},university:{required:true},description:{required:true},sort:{required:true,number:true},status:{required:true}';
